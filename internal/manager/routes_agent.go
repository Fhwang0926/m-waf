...
func (s *Server) registerAgentRoutes(mux *http.ServeMux) {
	...
	mux.Handle(protocol.PolicyBaseArtifactPattern, s.requireAgent(http.HandlerFunc(s.policyBaseArtifact)))
	mux.Handle(protocol.AgentPackagePattern, s.requireAgent(http.HandlerFunc(s.agentPackage)))
	mux.Handle(protocol.AgentUpgradePattern, s.requireAgent(http.HandlerFunc(s.createAgentUpgrade)))
	mux.Handle(protocol.AgentUpgradeCompletePattern, s.requireAgent(http.HandlerFunc(s.completeAgentUpgrade)))
	mux.Handle(protocol.EventBatchPattern, s.requireAgent(s.requireEventVerification(http.HandlerFunc(s.eventBatch))))
	mux.Handle(protocol.UnregisterPattern, s.requireAgent(http.HandlerFunc(s.unregisterAgent)))
