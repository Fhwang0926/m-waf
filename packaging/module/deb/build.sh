...
mkdir -p "$OUTPUT_DIR" "$METADATA_DIR" "$root/DEBIAN" "$root/etc/mwaf/active" "$root/usr/share/mwaf/crs" "$root/usr/share/doc/mwaf-modsecurity-$WEBSERVER" "$root/var/log/modsecurity"
...
case "$WEBSERVER" in
  apache)
    ...
    dependency='apache2, libapache2-mod-security2 (>= 2.9.7)'
  ...
  nginx)
    ...
    dependency='nginx (>= 1.24.0), libnginx-mod-http-modsecurity (>= 1.0.3)'
