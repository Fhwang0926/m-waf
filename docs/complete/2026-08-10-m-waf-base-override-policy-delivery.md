...
- 보호 정책 상세와 편집 화면에서 변경 대상이 시스템 정책이 아니라 기업별 설정임을 명시하고, 상속과 기업 설정 적용의 차이를 운영자용 문구로 정리했다.
- 상세 화면의 변경 버튼을 `기업 설정 변경`으로 바꾸고 시스템 정책은 변경되지 않는다는 안내와 버튼 줄바꿈 방지를 적용했다.
- 기업 설정 편집 화면에서 내부 호환용 hidden textarea가 공통 입력 스타일 때문에 노출되던 문제를 수정했다.
- 편집 흐름을 현재 적용 상태, 보호 방식 선택, 보존되는 사용자 정책, 변경 정보, 배포 결과 확인 순서로 재구성했다.
- 시스템 권장 설정과 기업 맞춤 설정을 비기술적인 설명으로 구분하고, 기업 맞춤 설정의 PL·점수·본문 검사는 고급 영역으로 정리했다.
